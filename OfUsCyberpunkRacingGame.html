...
<script>
     ...
     * Let Me Go - Sloped Roof, Neon, Rain, Multi-track Audio
     */
    ...
    function spawnObstacle() {
        ...
        // Anti-camping patch: Increased right lane spawn rate
        const rand = Math.random();
        let lane;
        if (rand < 0.3) lane = 0;       // 30% chance Left
        else if (rand < 0.6) lane = 1;  // 30% chance Center
        else lane = 2;                  // 40% chance Right
